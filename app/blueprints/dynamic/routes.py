...
from .generators import (
    apo as apoGenerator,
    prodrg as prodrgGenerator,
    acpype as acpypeGenerator,
)
from .executors import (
    apo as apoExecutor,
    prodrg as prodrgExecutor,
    acpype as acpypeExecutor,
)
import shutil
    ...
    try:
        if request.method == "POST":
            ...
            filename, ext = os.path.splitext(
                os.path.basename(request.files.get("file").filename)
            )
            folder = os.path.join(
                Config.UPLOAD_FOLDER, current_user.username, "apo", filename, timestamp
            )
            ...
            if request.form.get("execute") == "Executar":
                if upload_file(folder, request.files.get("file")):
                    ...
                    executing_path = os.path.join(
                        Config.UPLOAD_FOLDER, current_user.username, "executing"
                    )
                    if not os.path.exists(executing_path):
                        ...
                        flash(
                            _(
                                "Desculpe, não é possível realizar duas dinâmicas ao mesmo tempo."
                            ),
                            "danger",
                        )
                        return redirect(url_for("DynamicRoutes.apo"))

                    exc = apoExecutor.execute(
                        folder, CompleteFileName, current_user.username, filename + ext
                    )
                    flash(
                        _(
                            "Houve um erro ao executar o comando <b>%(command)s</b>.</br>Verifique os logs para mais detalhes",
                            command=exc,
                        ),
                        "danger",
                    )
        ...
        if CheckUserDynamics(current_user.username) == True:
            ...
            dynamicinfo = os.path.join(
                Config.UPLOAD_FOLDER, current_user.username, "running_protein_name"
            )
            ...
            log_dir_path = os.path.join(
                Config.UPLOAD_FOLDER, current_user.username, "log_dir"
            )
        ...
        return render_template("error.html", title=_("Ocorreu um erro."))
...
def prodrg():
    try:
        if request.method == "POST":
            ...
            fileitpname, ext1 = os.path.splitext(
                os.path.basename(request.files.get("fileitp").filename)
            )
            filegroname, ext2 = os.path.splitext(
                os.path.basename(request.files.get("filegro").filename)
            )
            folder = os.path.join(
                Config.UPLOAD_FOLDER,
                current_user.username,
                "prodrg",
                f"{filename}_{fileitpname}",
                timestamp,
            )
            ...
            if request.form.get("execute") == "Executar":
                if upload_file_ligante(
                    ...
                    request.files.get("fileitp"),
                    request.files.get("filegro"),
                ):  # dando upload no arquivo, salvando e checando
                    executing_path = os.path.join(
                        Config.UPLOAD_FOLDER, current_user.username, "executing"
                    )
                    if not os.path.exists(executing_path):
                        ...
                        flash(
                            "Não é permitido que o mesmo usuário realize duas dinâmicas simultâneas.",
                            "danger",
                        )
                        return redirect(url_for("DynamicRoutes.prodrg"))

                    exc = prodrgExecutor.execute(
                        folder,
                        CompleteFileName,
                        current_user.username,
                        name,
...
@DynamicBlueprint.route(
    "/cancel/<mode>/<protein>/<folder>", methods=["POST"], endpoint="cancel_dynamic"
)
@login_required
