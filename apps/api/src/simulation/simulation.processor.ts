...
export function isProcessRunning(pid: number): boolean {
  try {
    ...
    // kill(pid, 0) throws ESRCH when no such process exists, EPERM when the
    // process exists but we lack permission. Only ESRCH means the process is
    // gone; all other errors are treated as running.
